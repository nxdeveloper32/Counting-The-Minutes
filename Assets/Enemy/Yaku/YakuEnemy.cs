...
public class YakuEnemy : MonoBehaviour {
	...
	void Update () {
		...
		if (Vector3.Distance (Player.transform.position, transform.position) < 30 && (angle < 180 || pursuing)) {
	...
	IEnumerator HitPlayer(){
		...
		if (EnemyAttacking) {
			Player.GetComponent<Player_Health>().Hit(5);
		}
		hitplayer = true;
	}
